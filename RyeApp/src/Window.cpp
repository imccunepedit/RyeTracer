...
void Window::Run() {
  while (!glfwWindowShouldClose(m_windowHandle) && !m_shouldQuit) {
    ...
    vkWaitForFences(m_logicalDevice, 1, &m_inFlightFences[m_frameIndex],
                    VK_TRUE, UINT64_MAX);

    uint32_t imageIndex;
    VkResult result = vkAcquireNextImageKHR(
        m_logicalDevice, m_swapChain, UINT64_MAX,
        m_imageAvailableSemaphores[m_frameIndex], VK_NULL_HANDLE, &imageIndex);
    ...
    ImGui::DockSpaceOverViewport(
        ...
        ImGuiDockNodeFlags_PassthruCentralNode); // do the docking things, make
                                                 // sure whats rendered in the
    ...
    if (vkQueueSubmit(m_graphicsQueue, 1, &submitInfo,
                      m_inFlightFences[m_frameIndex]) != VK_SUCCESS)
...
void Window::InitGlfw() {
  ...
  m_windowHandle =
      glfwCreateWindow(WIDTH, HEIGHT, "m_floating Renderer", nullptr, nullptr);
...
void Window::CreateVulkanInstance() {
  if (enableValidationLayers && !CheckValidationLayerSupport())
    throw std::runtime_error(
        "ERROR: requested validation layers not available");
  ...
  createInfo.flags |=
      VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR; // on macos might be
                                                        // needed
...
void Window::CreateSurface() {
  if (glfwCreateWindowSurface(m_vulkanInstance, m_windowHandle, nullptr,
...
void Window::CreateFramebuffers() {
  ...
  for (size_t i = 0; i < m_swapChainImageViews.size(); i++) {
    ...
    if (vkCreateFramebuffer(m_logicalDevice, &framebufferInfo, nullptr,
...
void Window::CreateUniformBuffers() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    vkMapMemory(m_logicalDevice, m_uniformBuffersMemory[i], 0, bufferSize, 0,
...
void Window::CreateDescriptorPool() {
  ...
  if (vkCreateDescriptorPool(m_logicalDevice, &poolInfo, nullptr,
...
void Window::CreateDescriptorSets() {
  ...
  if (vkAllocateDescriptorSets(m_logicalDevice, &allocInfo,
...
void Window::CreateCommandBuffers() {
  ...
  if (vkAllocateCommandBuffers(m_logicalDevice, &allocInfo,
...
void Window::StartRecordCommandBuffer(VkCommandBuffer commandBuffer,
                                      uint32_t imageIndex) {
...
void Window::EndRecordCommandBuffer(VkCommandBuffer commandBuffer,
                                    uint32_t imageIndex) {
...
VkSurfaceFormatKHR Window::ChooseSwapSurfaceFormat(
...
VkPresentModeKHR Window::ChooseSwapPresentMode(
...
VkExtent2D
Window::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
...
std::vector<const char *> Window::GetRequiredExtensions() {
  ...
  extensions.emplace_back(
      VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME); // mac os maybe needs for
                                                      // compat
  if (enableValidationLayers) {
...
Window::DebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
